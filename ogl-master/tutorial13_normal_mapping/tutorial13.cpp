...
int main( void )
{
	...
	// Load the texture
	GLuint DiffuseTexture = loadDDS("resize3.DDS");
	GLuint NormalTexture = loadBMP_custom("resize3.bmp");
	GLuint SpecularTexture = loadDDS("resize3.DDS");
	...
	do{
		...
		glBegin(GL_LINES);
			...
			lightPos+=glm::vec3(1,0,0)*1.2f;
			glVertex3fv(&lightPos.x);
			lightPos-=glm::vec3(1,0,0)* 1.2f;
			glVertex3fv(&lightPos.x);
			lightPos+=glm::vec3(0,1,0)* 1.2f;
			glVertex3fv(&lightPos.x);
		glEnd();
